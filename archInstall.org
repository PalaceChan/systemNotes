...
# before chroot you want to have wifi already so do these
iwctl
station wlan0 scan
station wlan0 get-networks
station wlan0 connect your_ssid
exit # then confirm with a ping
...
in the case of an older setup like on the x220 with just sda1 partition, i still ran up to the first mount ~/dev/vg0/lv_root /mnt~ but then in the next block of commands i only did
#+begin_src bash
mount /dev/vg0/lv_home /mnt/home
#+end_src

the rest is the same but i skipped the efivarfs bit as not a uefi setup (~ls /sys/firmware/efi~ was empty)
